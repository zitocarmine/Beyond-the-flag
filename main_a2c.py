...
SAVE_FILE = "mario_a2c_pure.pth"
LOG_FILE = "training_log_a2c_pure.csv"
...
def train():
    print(f"Start Pure A2C Training on {DEVICE} with {NUM_ENVS} workers...")
    
    if not os.path.exists(LOG_FILE):
        with open(LOG_FILE, mode='w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(["Step", "Ext_Reward", "Max_X_Pos", "Entropy"])
    ...
    try:
        while step < TOTAL_STEPS:
            ...
            loss = policy_loss + 0.5 * value_loss - current_ent * entropy
            ...
            if step % 1000 == 0: 
                print(f"Step {step} | Reward: {mean_ext:.3f} | Max X: {max_x:.1f} | Ent: {current_ent:.3f}")
            
            with open(LOG_FILE, mode='a', newline='') as f:
                writer = csv.writer(f)
                writer.writerow([step, mean_ext, max_x, current_ent])
