...
MODEL_PATH = "mario_mastery.pth"  
DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")
FPS = 30  

def play():
    print(f"Starting gameplay of Mario PPO+ICM on {DEVICE}")
    
    env = create_mario_env(render=True)
    
    model = MarioPPO(env.action_space.n).to(DEVICE)
    model.eval() 
    ...
    if os.path.exists(MODEL_PATH):
        ...
        print(f"Model loaded {checkpoint.get('step', '?')} step)")
    else:
